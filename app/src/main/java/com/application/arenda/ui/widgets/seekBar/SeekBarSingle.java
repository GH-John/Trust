...
import com.application.arenda.ui.widgets.seekBar.interfaces.OnSeekbarFinalValueListener;

public class SeekBarSingle extends View {
    ...
    private SeekBarSingle.Thumb pressedThumb;
    ...
    public SeekBarSingle(Context context) {
        this(context, null);
    }

    public SeekBarSingle(Context context, AttributeSet attrs) {
    ...
    public SeekBarSingle(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public SeekBarSingle setLeftThumbHighlightColor(int leftThumbColorPressed) {
        this.leftThumbColorPressed = leftThumbColorPressed;
        return this;
    }

    public SeekBarSingle setLeftThumbDrawable(int resId) {
    ...
    public SeekBarSingle setLeftThumbDrawable(Drawable drawable) {
    ...
    public SeekBarSingle setLeftThumbBitmap(Bitmap bitmap) {
        leftThumb = bitmap;
        return this;
    }

    public SeekBarSingle setLeftThumbHighlightDrawable(int resId) {
        setLeftThumbHighlightDrawable(ContextCompat.getDrawable(getContext(), resId));
        return this;
    }

    public SeekBarSingle setLeftThumbHighlightDrawable(Drawable drawable) {
        setLeftThumbHighlightBitmap(getBitmap(drawable));
        return this;
    }

    public SeekBarSingle setLeftThumbHighlightBitmap(Bitmap bitmap) {
    ...
    public SeekBarSingle.Thumb getPressedThumb() {
        return pressedThumb;
    }
    ...
    public SeekBarSingle setCornerRadius(float cornerRadius) {
    ...
    public SeekBarSingle setMinValue(float minValue) {
    ...
    public SeekBarSingle setMaxValue(float maxValue) {
    ...
    public SeekBarSingle setMinStartValue(float minStartValue) {
    ...
    public SeekBarSingle setSteps(float steps) {
        this.steps = steps;
        return this;
    }
    ...
    public SeekBarSingle setBarColor(int barColor) {
    ...
    public SeekBarSingle setBarHighlightColor(int barHighlightColor) {
    ...
    public SeekBarSingle setLeftThumbColor(int leftThumbColorNormal) {
    ...
    public SeekBarSingle setDataType(int dataType) {
    ...
    public SeekBarSingle setPosition(int pos) {
    ...
    public Number getSelectedMinValue() {
        ...
        nv = (position == SeekBarSingle.Position.LEFT) ? nv : Math.abs(nv - maxValue);
    ...
    protected int getDataType(final TypedArray typedArray) {
        return typedArray.getInt(R.styleable.SeekbarSingleAndRange_data_type, SeekBarSingle.DataType.INTEGER);
    }

    protected final int getPosition(final TypedArray typedArray) {
        final int pos = typedArray.getInt(R.styleable.SeekbarSingleAndRange_position, SeekBarSingle.Position.LEFT);

        normalizedMinValue = (pos == SeekBarSingle.Position.LEFT) ? normalizedMinValue : normalizedMaxValue;
        return pos;
    }
    ...
    protected void setupHighlightBar(final Canvas canvas, final Paint paint, final RectF rect) {
        if (position == SeekBarSingle.Position.RIGHT) {
    ...
    protected void setupLeftThumb(final Canvas canvas, final Paint paint, final RectF rect) {
        ...
        leftThumbColor = (SeekBarSingle.Thumb.MIN.equals(pressedThumb)) ? leftThumbColorPressed : leftThumbColorNormal;
        ...
        if (leftThumb != null) {
            Bitmap lThumb = (SeekBarSingle.Thumb.MIN.equals(pressedThumb)) ? leftThumbPressed : leftThumb;
    ...
    protected void trackTouchEvent(MotionEvent event) {
        ...
        try {
            ...
            if (SeekBarSingle.Thumb.MIN.equals(pressedThumb)) {
    ...
    private SeekBarSingle.Thumb evalPressedThumb(float touchX) {
        SeekBarSingle.Thumb result = null;

        boolean minThumbPressed = isInThumbRange(touchX, normalizedMinValue);
        if (minThumbPressed) {
            // if both thumbs are pressed (they lie on top of each other), choose the one with more room to drag. this avoids "stalling" the thumbs in a corner, not being able to drag them apart anymore.
            result = SeekBarSingle.Thumb.MIN;
        }
        return result;
    }
    ...
    private double normalizedToValue(double normalized) {
        ...
        val = (position == SeekBarSingle.Position.LEFT) ? val + minValue : val;
        return val;
    }
    ...
    private <T extends Number> Number formatValue(T value) throws IllegalArgumentException {
        ...
        if (dataType == SeekBarSingle.DataType.LONG) {
            return v.longValue();
        }
        if (dataType == SeekBarSingle.DataType.DOUBLE) {
            return v;
        }
        if (dataType == SeekBarSingle.DataType.INTEGER) {
            return Math.round(v);
        }
        if (dataType == SeekBarSingle.DataType.FLOAT) {
            return v.floatValue();
        }
        if (dataType == SeekBarSingle.DataType.SHORT) {
            return v.shortValue();
        }
        if (dataType == SeekBarSingle.DataType.BYTE) {
            return v.byteValue();
        }
